  ...
  const changeJobVisiblity = async (id) => {
    try {
      ...
      if (data.success) {
        ...
        fetchCompanyJobs(); // Refresh jobs list after visibility change
      } else {
        toast.error(data.message);
      }
    } catch (error) {
              ...
              <tr key={index} className='text-gray-700'>
                ...
                <td className='py-2 px-4 border-b'>{job.title}</td>
                 ...
                 <td className='py-2 px-4 border-b '>
                  <input onChange={()=>changeJobVisiblity(job._id)} className='scale-125 ml-4' type="checkbox" checked={job.visible} readOnly />
                 </td>
