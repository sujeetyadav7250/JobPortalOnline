...
const JobListing = () => {
    ...
    const [selectedCatogries, setSelectedCategories] = useState([])
    ...
    useEffect(() =>{
      const matchesCategory = job => selectedCatogries.length === 0 || selectedCatogries.includes(job.category)
    ...
    },[jobs,selectedCatogries, selectedLocations,searchFilter])


  return (
    <div className='container 2xl:px-20 mx-auto flex flex-col lg:flex-row max-lg:space-y py-8'>
      {/* Sidebar */}
          ...
          <ul className='space-y-4 text-gray-600'>
            {
              JobCategories.map((category,index)=>(
                <li className='flex gap-3 items-center' key={index}>
                   ...
                   checked = {selectedCatogries.includes(category)} 
                   />
                   {category}
                </li>
              ))
